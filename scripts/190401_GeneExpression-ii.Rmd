...
cleaned_data <- original_data %>% 
  # Separate takes the NAME column and splits it into five columns.
  separate(NAME, c("name", "BP", "MF", "systematic_name", "number"), sep = "\\|\\|") %>%
  # Trims the white space
  mutate_at(c("name", "BP", "MF", "systematic_name"), trimws) %>% 
  # Selecting all columns except number, GID, YORF, GWEIGHT
  select(-number, -GID, -YORF, -GWEIGHT) %>%
  # Gather takes the columns from G0.05 to U0.3 and puts the column headings into the "sample" column and the values into "expression"
  gather(sample, expression, G0.05:U0.3) %>%
  # Separate takes the sample column and splits it into two columns "nutrient" and rate", splits after the first character
  separate(sample, c("nutrient", "rate"), sep = 1, convert = TRUE) %>%
  # Recodes the values in the nutrient column so that we're only dealing with the letter G, L, P, S, N, and U.
  mutate(nutrient = recode(nutrient, G = "Glucose", L = "Leucine", 
                           P = "Phosphate", S = "Sulfate", N = "Ammonia", U = "Uracil")) %>%
  # Get rid of the rows that have missing values, or where systematic_name is empty.
